...
for index, row in data.iterrows():
    sites[index] = set()
    allsites[index] = set()
    site_assignment = row['Site_Assignment']
    for glycan in site_assignment.split(";"):
        if "HexNAc" not in glycan:
            continue
        if 'Oxidation@' in glycan:
            linkages = glycan.replace('Oxidation@', '').replace('HexNAc@', '')
        else:
            linkages = glycan.split("@")[1]
        if "=" in linkages:
            site = linkages.split("=")[0]
            sites[index].add(site)
            allsites[index].add(site)
        elif "|" in linkages:
            for j in linkages.split("|"):
                if "&" in j:
                    for k in j.split("&"):
                        allsites[index].add(k)
                else:
                    allsites[index].add(j)
        else:
            sites[index].add(linkages)
            allsites[index].add(linkages)

# add sites to dataframe as a list
...
#     sites[key] = ','.join(item)
# for key, item in allsites.items():
#     allsites[key] = ','.join(item)
# print(sites)
# print(allsites)
